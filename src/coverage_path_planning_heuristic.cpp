...
namespace SSPP
{
    static vector<Node>  usefulNode;
    static double  preCov = 0.0;
    static int usefulNodeNum = 0;
...
//     meshSurface          = new MeshSurface(nh);
    ...
    gradualCloudPub  = nh.advertise<sensor_msgs::PointCloud2>("gradual_point_cloud", 100);

    clusterPub        = nh.advertise<sensor_msgs::PointCloud2>("cluster_cloud", 100);

    accuracySum          = 0.0;
    extraCovSum          = 0.0;
    extraAreaSum         = 0.0;
    volumetricVoxelRes   = 0.5;//0.5
    accW                 = 0.8;//0.8
    distW                = 0.5;//0.5
    covW                 = 1;
    angleW               = 0.2;//0.2
    ...
    for (int kk = min_b.z (); kk <= max_b.z (); ++kk)
    {
        for (int jj = min_b.y (); jj <= max_b.y (); ++jj)
        {
            for (int ii = min_b.x (); ii <= max_b.x (); ++ii)
            {
                ...
                if(index1!=-1)
                {//占用
                    Eigen::Vector4f centroid = originalCloudFilteredVoxels.getCentroidCoordinate (ijk);//体素在原始图中的体素中心点坐标
                    //求覆盖图中序号ijk
...
{
    //divide the uncovered parts into subregions (using euclidean clustering)
    ...
    ec.setClusterTolerance (0.6); // 30cm//0.8
...
//      std::cout << "PointCloud representing the Cluster: " << cloud_cluster->points.size () << " data points." << std::endl;
    ...
    pcl::toROSMsg(*coloredClusters, clusterMsg);
    clusterMsg.header.stamp = ros::Time::now();
    clusterMsg.header.frame_id = "map"; //change according to the global frame please!!
    clusterPub.publish(clusterMsg);
}
...
void CoveragePathPlanningHeuristic::findClusterBB(pcl::PointCloud<pcl::PointXYZ> clusterPoints, geometry_msgs::Vector3& gridSize, geometry_msgs::Pose& gridStart)
{
    ...
    double maximizeSizeXY = 2;//5
    double maximizeSizeZ = 2;//3
...
Eigen::Vector3d toEulerAngle(const double x,const double y,const double z,const double w)
    {
...
bool CoveragePathPlanningHeuristic::terminateConditionReached(Node *node)
{
    ...
    if (usefulNode.empty() || node->coverage > preCov){
        usefulNode.push_back(node);
        usefulNodeNum++;
        ofstream pathFile;
        std::string file_loc = "/home/janewin/Desktop/usefulNode.csv";
        pathFile.open (file_loc.c_str(), ios::in | ios::app);
        Eigen::Vector3d ret = toEulerAngle(node->senPoses[0].p.orientation.x, node->senPoses[0].p.orientation.y, node->senPoses[0].p.orientation.z, node->senPoses[0].p.orientation.w);
        pathFile << usefulNodeNum<<","<<node->pose.p.position.x<<","<<node->pose.p.position.y<<","<<node->pose.p.position.z<<","<<ret[2]<<","<<ret[1]<<"\n";
        pathFile.close();
    }
    preCov = node->coverage;
    ...
    {
        double avgAcc = accuracySum/accuracyPerViewpointAvg.size();
        std::cout<<"\n\nAverage Accuracy per viewpoint is "<<avgAcc<<" / "<<std::sqrt(avgAcc / 0.0000285)<<std::endl;
        std::cout<<"Average extra coverage per viewpoint is "<<extraCovSum/extraCovPerViewpointAvg.size()<<std::endl;
    ...
    if(intersectionsCount%2 != 1)
    {
        ...
        if (sqd >=(minDist*minDist) && sqd <= (maxDist*maxDist) )
        {
            ...
            for(int i=0; i<correspondingSensorPoses.poses.size(); i++)
            {
                ...
                pcl::PointCloud<pcl::PointXYZ> accuracyPts;
...
//                    globalCloud += pts;
                    ...
                    occlussionCulling->getAccuracyCloud(pts,correspondingSensorPoses.poses[i], accuracyPts, accuracyThreshhold);
                    if(accuracy >= accuracyThreshhold)
                    {
//                        accuracyClusters.push_back(pts);
//                        accuracyClusters.push_back(accuracyPts);

                        return false;
                    }
                    globalCloud += accuracyPts;
                }
...
//                collectiveCloud.points = node->parent->cloud_filtered->points;
//                collectiveCloud +=visibleCloud;
//                node->cloud_filtered->points = collectiveCloud.points;
...
//                std::cout<<"Total Entroby: "<<node->totalEntroby<<std::endl;
            ...
            //if node is at the same position of the parent with different orientation
...
void CoveragePathPlanningHeuristic::displayGradualProgress(Node *node)
{
    ...
    for(int i =0; i<node->senPoses.size();i++){
        geometry_msgs::Pose temp = node->senPoses[i].p;
        selectedSensorPoses.poses.push_back(temp);
    }
    ...
    pcl::PointCloud<pcl::PointXYZ> temp_cloud_gradual, combined_gradual;
    //########display FOV##########
    //        if (coveI != 0 && %10==0)
//    {


    pcl::PointCloud<pcl::PointXYZ>::Ptr CloudPtr_gradual(new pcl::PointCloud<pcl::PointXYZ>);
        for(int i =0; i<node->senPoses.size(); i++){
            occlussionCulling->visualizeFOV(node->senPoses[i].p, i);
            temp_cloud_gradual = occlussionCulling->extractVisibleSurface(node->senPoses[i].p);
            combined_gradual += temp_cloud_gradual;
        }
    sensor_msgs::PointCloud2 cloud_gradual;
    CloudPtr_gradual->points=combined_gradual.points;
    pcl::toROSMsg(*CloudPtr_gradual, cloud_gradual); //cloud of original (white) using original cloud
    cloud_gradual.header.stamp = ros::Time::now();
    cloud_gradual.header.frame_id = "map"; //change according to the global frame please!!
    gradualCloudPub.publish(cloud_gradual);
...
//    }
    ...
    if ( coveI%1==0)
    {
        ...
        std::string fileloc = path+ "/text/path_testfile.txt";
...
geometry_msgs::Pose CoveragePathPlanningHeuristic::findBestPitch(Sensors &rSensor, geometry_msgs::Pose pose, pcl::PointCloud<pcl::PointXYZ>& globalCloud) {
    float pitch = -1, bestPitch = 0;
    float minAcc = 44654654.0;
    Eigen::Vector3f originRPY, bestRPY, outRPY;
    geometry_msgs::Pose sensorLoc, bestSensorLoc = rSensor.robot2sensorTransformation(pose);
    ...
    if(intersectionsCount%2 != 1) {
        ...
        if (sqd >= 1 && sqd <= 16) {
            while (pitch<=90){
                originRPY = rSensor.getSensorRpy();
                bestRPY[0]= 0;
                bestRPY[1] = pitch / 180 * 3.14159;
                bestRPY[2] = 0;
                rSensor.setSensorRpy(bestRPY);
                sensorLoc = rSensor.robot2sensorTransformation(pose);
                pcl::PointCloud<pcl::PointXYZ> pts;
//                geometry_msgs::Pose location;
//                location.position = sensorLoc.position;
//                location.orientation = sensorLoc.orientation;
                pts = occlussionCulling->extractVisibleSurface(sensorLoc);
                if(pts.size()>5)
                {
                    double stdDepth = occlussionCulling->calcAvgAccuracy(pts,sensorLoc);
                    if (stdDepth < minAcc){
                        stdDepth = minAcc;
                        bestSensorLoc = sensorLoc;
//                        std::cout<<"已优化!"<<pitch<<std::endl;
                    }
                }
                pitch += 30;
                rSensor.setSensorRpy(originRPY);
            }
        }
    }
    return bestSensorLoc;
}

geometry_msgs::Pose CoveragePathPlanningHeuristic::findBestOri(Sensors &rSensor, geometry_msgs::Pose& pose, pcl::PointCloud<pcl::PointXYZ>& globalCloud) {
        float  bestPitch = 0;
        float minAcc = 44654654.0;
        Eigen::Vector3f originRPY, bestRPY, outRPY;
        geometry_msgs::Pose sensorLoc, bestSensorLoc = rSensor.robot2sensorTransformation(pose);
        ...
        if(intersectionsCount%2 != 1) {
            ...
            if (sqd >= 1 && sqd <= 16) {
                ...
                for (float pitch = -1; pitch < 90; pitch += 30) {
                    for (double yaw = 0.0; yaw < 2*M_PI; yaw+=(45*M_PI/180.0f)) {
                        const tf::Quaternion &tf22 = tf::createQuaternionFromYaw(yaw);
                        pose.orientation.x = tf22.getX();
                        pose.orientation.y = tf22.getY();
                        pose.orientation.z = tf22.getZ();
                        pose.orientation.w = tf22.getW();
